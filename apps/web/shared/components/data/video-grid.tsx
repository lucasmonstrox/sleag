...
/** Layout do grid de vídeos — compartilhado pelo VideoGrid e pela island virtualizada. */
export const VIDEO_GRID_COLS =
  "grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6"

/**
 * Card único de vídeo (thumb + título + criador + métricas). `onPlay` abre o
 * player no pai (o modal vive fora do card pra sobreviver à virtualização, que
 * desmonta cards fora da viewport). `seed` varia o gradiente do placeholder.
 */
export function VideoCard({
  ...
  return (
    <div className="flex flex-col gap-2.5">
      <button
        type="button"
        onClick={() => item.videoId && onPlay(item)}
        ...
        className={cn(
          ...
          !item.cover && THUMB_TONES[seed % THUMB_TONES.length],
...
/** Lightbox do player do TikTok, controlado pelo pai (active/onClose). */
export function VideoLightbox({
  active,
  onClose,
}: {
  active: VideoItem | null
  onClose: () => void
}) {
  return (
...
export function VideoGrid({ items, className }: VideoGridProps) {
  ...
  return (
    <>
      <div className={cn("grid", VIDEO_GRID_COLS, className)}>
